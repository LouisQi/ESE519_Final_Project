...
void intToCharArray(double number, char* charArray) {
    char temp[20];
    dtostrf(number,3,2,temp);
    sprintf(charArray, "%s\n\r", temp);
}

void transend(double number1, double number2, char* charArray) {
    char temp1[20];
    char temp2[20];
    dtostrf(number1, 3, 2, temp1);
    dtostrf(number2, 3, 2, temp2);
    sprintf(charArray, "%s,%s\n\r", temp1, temp2); // Format with both numbers separated by a comma
...
int main(void) {
  ...
  double number1 = 34.5; // First number
  double number2 = 0.86; // Second number
  char charArray[40]; // Adjust the array size as needed
  
  while (1) {
    transend(number1, number2, charArray); // Send both numbers over UART
